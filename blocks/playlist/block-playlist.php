...
<?php if ($is_preview): ?>
	<div class="block-preview-message">
		<p><?php _e('Playlist', 'mars'); ?></p>
		<?php if (empty($tracks)): ?>
			<h2><?php _e('Aucune piste dans la playlist. Veuillez ajouter des pistes dans les paramètres du bloc.', 'mars'); ?></h2>
		<?php else: ?>
			<ul>
				<?php foreach ($tracks as $index => $track): ?>
					<li><?php echo $track['titre']; ?></li>
				<?php endforeach; ?>
			</ul>
		<?php endif; ?>
	</div>
